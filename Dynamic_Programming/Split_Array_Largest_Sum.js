...
var splitArray = function (nums, m) {
    ...
    const helper = (i, arrays, sum, max) => {

        if (i === nums.length && arrays === m) {
        ...
        if (i > 0) {
            let tempmax = Math.max(max, sum + nums[i])
            helper(i + 1, arrays, sum + nums[i], tempmax)
        }

        if (arrays < m) {
            let tempmax = Math.max(max, nums[i])
            helper(i + 1, arrays + 1, nums[i], tempmax)
        }
...
var dpSplitArray = (nums, m) => {

    const n = nums.length;

    let dp = new Array(n + 1).fill(0).map(_ => new Array(m + 1).fill(Infinity));
    ...
    for (let i = 1; i <= n; i++) {
        for (let j = 1; j <= m; j++) {
            for (let k = 0; k < i; k++) {
                console.log(Math.max(dp[k][j - 1], sub[i] - sub[k]))
