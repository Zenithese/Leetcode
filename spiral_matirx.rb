...
def spiral_order(matrix, spiral = [], visited = Set.new)
    ...
    if matrix.length > 0
        while spiral.length < matrix.length * matrix[0].length
            ...
            direction = (direction + 1) % 4 if (row == matrix.length - 1 && direction == 1) || (col == matrix[0].length - 1 && direction == 0) || (row == 0 && direction == 3) || (col == 0 && direction == 2) || visited.include?("#{row + directions[direction][0]}-#{col + directions[direction][1]}")
